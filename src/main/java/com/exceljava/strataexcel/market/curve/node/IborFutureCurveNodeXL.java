...
public class IborFutureCurveNodeXL {
    private final ExcelAddIn xl;

    public IborFutureCurveNodeXL(ExcelAddIn xl) {
        this.xl = xl;
    }
    
    @ExcelFunction(
        value = "og.IborFutureCurveNode.build",
        category = "Strata",
        isThreadSafe = true
    )
    @ExcelArguments({
    ...
    public IborFutureCurveNode builder(String[] keys, Object[] values) {
        ...
        IborFutureCurveNode.Builder builder = IborFutureCurveNode.builder();
        
        Object template = args.get("template");
        if (null != template) {
            IborFutureTemplate value;
            try {
                value = xl.convertArgument(template, IborFutureTemplate.class);
            } catch (Exception e) {
                throw new IllegalArgumentException("template could not be converted to IborFutureTemplate", e);
            }
        ...
        if (null != rateId) {
            QuoteId value;
            try {
                value = xl.convertArgument(rateId, QuoteId.class);
            } catch (Exception e) {
                throw new IllegalArgumentException("rateId could not be converted to QuoteId", e);
            }
    ...
    @ExcelFunction(
        value = "og.IborFutureCurveNode.ofRate",
        category = "Strata",
        isThreadSafe = true
    )
    @ExcelArguments({
    ...
    public IborFutureCurveNode of(IborFutureTemplate template, QuoteId rateId) {
        return IborFutureCurveNode.of(template, rateId);
    }

    @ExcelFunction(
        value = "og.IborFutureCurveNode.ofSpread",
        category = "Strata",
        isThreadSafe = true
    )
    @ExcelArguments({
    ...
    public IborFutureCurveNode of(IborFutureTemplate template, QuoteId rateId, double additionalSpread) {
        return IborFutureCurveNode.of(template, rateId, additionalSpread);
    }

    @ExcelFunction(
        value = "og.IborFutureCurveNode.toString",
        category = "Strata",
        isThreadSafe = true
    )
    @ExcelArguments({
        @ExcelArgument("iborFutureCurveNode")
    })
    public String toString(IborFutureCurveNode iborFutureCurveNode) {
        return iborFutureCurveNode.toString();
    }
}
